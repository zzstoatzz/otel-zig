...
pub fn StandardCounter(comptime T: type) type {
    return struct {
        ...
        pub fn add(self: *@This(), ctx: Context, value: T, attributes: []const KeyValue) void {
            _ = ctx;
            _ = attributes;
...
pub fn StandardGauge(comptime T: type) type {
    return struct {
        ...
        pub fn record(self: *@This(), ctx: Context, value: T, attributes: []const KeyValue) void {
            _ = ctx;
            _ = attributes;
            
            // For MVP, we ignore attributes and just record the last value
            self.mutex.lock();
            defer self.mutex.unlock();
...
test "StandardUpDownCounter operations" {
    ...
    counter.add(ctx, -3.2, &attrs);
    counter.add(ctx, 5.7, &attrs);
...
test "StandardGauge operations" {
    ...
    gauge.record(ctx, 24.1, &attrs);
    try testing.expectEqual(@as(f64, 24.1), gauge.getValue().?);
    
    gauge.record(ctx, 22.8, &attrs);
